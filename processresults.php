...
foreach(scandir($directory) as $file) {
	...
	if (isset($json['Server'])) {
		...
		if ($database->has("serverinfo", ["AND" => ["Time" => $servertime], "Server" => $json['Server']])) {
			...
			$last_server_id = $database->insert("serverinfo", [
				"Server" => $json['Server'], 
				"Time" => $servertime, 
				"Experience" => $json['Experience'], 
				"Difficulty" => $json['Difficulty'], 
				"FuelUsage" => $json['FuelUsage'], 
				"TireWear" => $json['TireWear'], 
				"MechanicalDamage" => $json['MechanicalDamage'], 
				"FlagRules" => $json['FlagRules'], 
				"CutRules" => $json['CutRules'], 

				"WreckerPrevention" => $json['WreckerPrevention'], 
				"MandatoryPitstop" => $json['MandatoryPitstop'], 
				"Track" => $json['Track']
			]);
			...
			foreach($sessions as & $session) {
				...
				$last_session_id = $database->insert("sessions", [
					"Type" => $session['Type'], 
				...
				echo "Unranked Session";
					...
					foreach($players as & $player) {
						...
						if ($database->has("players", ["Username" => $puser])) {
							...
							$database->insert("sessionplayers", [
						...
						else 
						{
							echo "Adding New User" . $puser, "</br>";
								$last_player_id = $database->insert("players", [
								"FullName" => $player['FullName'], 
								"Username" => $puser]);
								$database->insert("sessionplayers", [
								"FullName" => $player['FullName'], 
					...
					foreach($players as & $player) {
						...
						if ($database->has("players", ["Username" => $puser])) {
							...
							foreach($laps as $lap) {
								...
								$database->insert("racesessionlaps", [
									"PlayerID" => $last_player_id, 
									"SessionID" => $last_session_id, 
									"Time" => $ltime, 
									"Position" => $lap['Position'], 
									"PitStopOccured" => $lap['PitStopOccured']
									]);
							}
						}
					}
